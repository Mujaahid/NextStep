$(document).ready(function() {
    ...
    $(document).on('click', '.videoWrapper', function(event) {
        // Check if the click event occurred on the video element or its play/pause buttons
        let target = event.target;
        let video = document.getElementById('video');
        let playButton = document.querySelector('.playButton');
        let pauseButton = document.querySelector('.pauseButton');
      
        if (target === video || target === playButton || target === pauseButton) {
          // Call the togglePlay() function
          togglePlay(video, playButton, pauseButton);
        }
      });
      
      // Your togglePlay() function with video, playButton, and pauseButton as parameters
      function togglePlay(video, playButton, pauseButton) {
        if (video.paused) {
          video.play();
...
function loadVideo(index) {
  const $video = $('#your-video-id'); // Replace 'your-video-id' with the actual ID of your video element
  ...
  $video.on('loadedmetadata', function() {
    // Update the duration span when the video is loaded
    duration.text(formatTime($video[0].duration)); // Use $video[0] to access the actual video element
  });

  // Update current time and progress when the video updates
  $video.on('timeupdate', function() {
    currentTime.text(formatTime($video[0].currentTime)); // Use $video[0] to access the actual video element
    let progress = ($video[0].currentTime / $video[0].duration) * 100; // Use $video[0] to access the actual video element
    $progressBar.css('width', progress + '%');
  });
}

// Call the loadVideo function with the index of the video you want to load
loadVideo(0); // Load the first video in the array
